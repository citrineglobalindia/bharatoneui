              ...
              <tr key={r.id} className="border-t border-border align-top">
                ...
                <td className="px-3 py-3">
                  <div className="flex flex-wrap justify-end gap-1.5">
                    ...
                    {role === "admin" && <>
                      {r.status !== "qc_review" && <Button size="sm" variant="outline" className="h-8" disabled={busy === r.id} onClick={async () => { setBusy(r.id); const { error } = await supabase.rpc("resend_to_qc", { reg_id: r.id, notes: null }); setBusy(null); if (error) { toast.error(error.message); return; } toast.success("Sent for re-QC"); load(); }}>
                        <RefreshCw className="h-3.5 w-3.5" /> Re-QC
                      </Button>}
