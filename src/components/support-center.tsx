...
import { LifeBuoy, Phone, Mail, MessageSquare, Plus, Loader2, RefreshCw, ChevronRight, X, Send } from "lucide-react";
import { Button } from "@/components/ui/button";
import { supabase } from "@/integrations/supabase/client";
import { useCurrentUser } from "@/lib/use-current-user";
import { SupportThread } from "@/components/support-thread";

type Ticket = { id: string; ticket_no: string; category: string | null; priority: string | null; subject: string; body: string | null; status: string; assigned_name: string | null; created_at: string };
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-3 lg:grid-cols-3">
        {[["Call us", "1800 123 4567 · 8am–10pm", Phone, "bg-india-green/10 text-india-green"], ["Email", "help@bharatone.in", Mail, "bg-blue-500/10 text-blue-600"], ["Live Chat", "Avg response 2 min", MessageSquare, "bg-violet-500/10 text-violet-600"]].map(([t, d, Icon, tone]: any, i) => (
      ...
      <div className="grid gap-5 lg:grid-cols-[360px_1fr] items-start">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          ...
          {loading ? <div className="py-8 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-muted-foreground" /></div>
            : rows.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">No tickets yet. Raise one to get help.</p>
