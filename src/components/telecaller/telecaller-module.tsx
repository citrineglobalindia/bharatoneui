...
export function TelecallerModule() {
  ...
  return <TelecallerShell activeSection={activeSection} onSectionChange={setActiveSection}><div className="mx-auto max-w-[1600px] space-y-6">
    ...
    <Tabs value={activeTab} onValueChange={setActiveSection} className="space-y-4">
      ...
      <TabsContent value="followups"><div className="grid gap-4 xl:grid-cols-3"><section className="rounded-2xl border border-border bg-card p-5 shadow-soft xl:col-span-2"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-saffron">Action queue</p><h2 className="font-display text-lg font-extrabold">Today’s follow-ups</h2></div><Badge className="bg-saffron text-primary-foreground">9 due</Badge></div><div className="mt-4 divide-y divide-border">{leads.filter((lead) => ["Interested","Follow-up Required","Documents Pending"].includes(lead.status)).map((lead) => <div key={lead.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-saffron/10 text-saffron"><AlarmClock className="h-5 w-5"/></span><div className="min-w-0 flex-1"><p className="font-bold">{lead.name}</p><p className="text-xs text-muted-foreground">{lead.service} · {lead.nextAction} · {lead.owner}</p></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => toast.success("Reminder sent", { description: `WhatsApp reminder queued for ${lead.name}.` })}><MessageCircle/> Remind</Button><Button size="sm" onClick={() => callLead(lead)}><Phone/> Call</Button></div></div>)}</div></section><aside className="rounded-2xl border border-border bg-card p-5 shadow-soft"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-hr">Reminder health</p><h2 className="font-display text-lg font-extrabold">Schedule coverage</h2><div className="mt-5 space-y-5">{[["Due now",9,36],["Later today",11,44],["Tomorrow",5,20]].map(([label,count,value]) => <div key={label as string}><div className="mb-2 flex justify-between text-xs"><span className="font-semibold">{label as string}</span><span className="text-muted-foreground">{count as number}</span></div><Progress value={value as number} className="[&>div]:bg-hr"/></div>)}</div><div className="mt-6 rounded-xl bg-hr-soft p-4"><p className="text-xs font-bold text-hr">No missed reminder risk</p><p className="mt-1 text-[11px] text-muted-foreground">All high-priority leads have a next action scheduled.</p></div></aside></div></TabsContent>

      <TabsContent value="reporting"><div className="grid gap-4 xl:grid-cols-5"><section className="rounded-2xl border border-border bg-card p-5 shadow-soft xl:col-span-3"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-hr">14 June 2026</p><h2 className="font-display text-lg font-extrabold">Daily reporting dashboard</h2></div><Button variant="outline" onClick={() => toast.success("Report exported")}><BarChart3/> Export</Button></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">{FUNNEL.map(([label,count], index) => <div key={label} className="relative rounded-xl border border-border bg-background p-4"><p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">{label}</p><p className="mt-2 text-2xl font-extrabold">{count}</p>{index < FUNNEL.length - 1 && <ArrowRight className="absolute -right-3 top-1/2 z-10 hidden h-5 w-5 -translate-y-1/2 rounded-full bg-card text-muted-foreground sm:block"/>}</div>)}</div></section><aside className="rounded-2xl border border-border bg-card p-5 shadow-soft xl:col-span-2"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-india-green">Conversion intelligence</p><h2 className="font-display text-lg font-extrabold">Pipeline quality</h2><div className="mt-5 space-y-4">{[["Connection rate","61%",61],["Interest rate","32%",32],["Application rate","29%",29],["Activation rate","50%",50]].map(([label,value,percent]) => <div key={label as string}><div className="mb-1.5 flex justify-between text-xs"><span className="font-semibold">{label as string}</span><span className="font-bold text-india-green">{value as string}</span></div><Progress value={percent as number} className="[&>div]:bg-india-green"/></div>)}</div></aside></div></TabsContent>
