...
function Sidebar({ active, onSelect, close }: { active: string; onSelect: (id: string) => void; close?: () => void }) {
  ...
  return <div className="flex h-full flex-col bg-navy text-bd-foreground">
    ...
    <div className="border-t border-bd-foreground/10 p-3"><Button variant="ghost" className="w-full justify-start text-bd-foreground/70 hover:bg-bd-foreground/10 hover:text-bd-foreground" onClick={() => { localStorage.removeItem("bharatone:auth"); navigate({ to: "/login", replace: true }); }}><LogOut />Sign out</Button></div>
  </div>;
}
