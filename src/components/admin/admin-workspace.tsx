...
import {
  ...
  MessageSquare, IdCard, Wallet, Network,
  type LucideIcon,
} from "lucide-react";
import {
...
import { LiveOperations, RevenueAnalytics, Settlements, RiskFraud } from "@/components/admin/admin-insights";
...
const NAVIGATION: NavGroup[] = [
  ...
  { label: "Network", items: [{ label: "User Management", icon: Users }, { label: "KYC Approvals", icon: ClipboardCheck, badge: "116" }, { label: "DRO Operations", icon: MapPin }, { label: "TRO Operations", icon: MapPin }, { label: "Applications", icon: FileSearch }, { label: "Retailer Network", icon: Store }, { label: "Distributors", icon: Network }, { label: "Distributor Apps", icon: ClipboardCheck }, { label: "Retailer Map", icon: MapPin }, { label: "Roles & Permissions", icon: LockKeyhole }] },
  { label: "Finance", items: [{ label: "Accountant Operations", icon: Landmark }, { label: "Revenue Analytics", icon: BarChart3 }, { label: "Settlements", icon: Landmark }, { label: "Application Ledger", icon: WalletCards }, { label: "Wallet & Top-ups", icon: Wallet }, { label: "Withdrawals", icon: WalletCards }, { label: "Risk & Fraud", icon: ShieldCheck, badge: "8" }] },
  { label: "Platform", items: [{ label: "Service Catalog", icon: Zap }, { label: "Old JSKO IDs", icon: IdCard }, { label: "Support Center", icon: Headphones }, { label: "Live Chat", icon: MessageSquare }, { label: "System Settings", icon: Settings }, { label: "Feedback", icon: MessageSquare }, { label: "Audit Log", icon: FileClock }] },
];
...
export function AdminWorkspace() {
  ...
  useEffect(() => { (async () => { await ensureStaffSession(); const { data } = await supabase.rpc("admin_dashboard"); setDash(data ?? null); })(); }, []);
  const inrShort = (n: number) => { n = Number(n || 0); if (n >= 1e7) return "\u20b9" + (n/1e7).toFixed(2) + "Cr"; if (n >= 1e5) return "\u20b9" + (n/1e5).toFixed(2) + "L"; if (n >= 1e3) return "\u20b9" + (n/1e3).toFixed(1) + "K"; return "\u20b9" + n; };
  const kpis = useMemo(() => {
    if (!dash) return KPI;
    const total = Number(dash.applications || 0), ok = Number(dash.approved || 0);
    return [
      { label: "Gross volume", value: inrShort(dash.gross_total), detail: inrShort(dash.gross_today) + " today", icon: BadgeIndianRupee, tone: "admin" },
      { label: "Retailers", value: Number(dash.retailers||0).toLocaleString("en-IN"), detail: (dash.active_retailers||0) + " active", icon: Store, tone: "success" },
      { label: "Wallet pool", value: inrShort(dash.wallet_pool), detail: "across wallets", icon: WalletCards, tone: "warning" },
      { label: "Success rate", value: total ? Math.round((ok/total)*100) + "%" : "\u2014", detail: total + " applications", icon: CircleCheckBig, tone: "success" },
      { label: "KYC pending", value: Number(dash.kyc_pending||0).toLocaleString("en-IN"), detail: "awaiting review", icon: ClipboardCheck, tone: "warning" },
      { label: "Open tickets", value: Number(dash.open_tickets||0).toLocaleString("en-IN"), detail: "support queue", icon: AlertTriangle, tone: "danger" },
    ];
  }, [dash]);
  const servicesData = useMemo(() => {
    const fills = ["var(--admin)", "var(--india-green)", "var(--saffron)", "var(--admin-muted)", "#8b5cf6", "#06b6d4"];
    const arr = (dash?.service_dist as any[]) ?? [];
    if (!arr.length) return SERVICES;
    return arr.map((x, i) => ({ name: x.name, value: Number(x.value), fill: fills[i % fills.length] }));
  }, [dash]);
  const recentRows = useMemo(() => ((dash?.recent as any[]) ?? []).map((r) => ({ id: r.application_no, service: r.service_name, user: r.submitter_name || "\u2014", location: r.status, amount: "\u20b9" + Number(r.service_charge||0).toLocaleString("en-IN"), status: (r.status === "approved" || r.status === "completed") ? "Success" : r.status === "rejected" ? "Review" : "Processing", time: new Date(r.created_at).toLocaleDateString("en-IN") })), [dash]);
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-[1700px] space-y-5 p-4 lg:p-6 xl:p-7">
            ...
            <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">{kpis.map((item) => <MetricCard key={item.label} item={item} />)}</section>
            ...
            </> : active === "Audit Log" ? <AdminAuditLog /> : active === "User Management" ? <AdminUsers /> : active === "Service Catalog" ? <CatalogManager /> : active === "Old JSKO IDs" ? <JskoManager /> : active === "Applications" ? <ServiceApplicationsView /> : active === "Application Ledger" ? <ApplicationLedger /> : active === "Wallet & Top-ups" ? <WalletAdmin /> : active === "Withdrawals" ? <WithdrawalsAdmin /> : active === "Accountant Operations" ? <RegistrationsReview /> : active === "KYC Approvals" ? <RegistrationsReview /> : active === "DRO Operations" ? <RegionalOperations role="DRO" /> : active === "TRO Operations" ? <RegionalOperations role="TRO" /> : active === "Retailer Network" ? <RetailerNetworkCenter /> : active === "Distributors" ? <DistributorAdmin /> : active === "Distributor Apps" ? <DistributorApplications /> : active === "Retailer Map" ? <RetailerMap scope="admin" /> : active === "System Settings" ? <SystemSettings /> : active === "Support Center" ? <SupportAdmin /> : active === "Live Chat" ? <LiveChatAdmin /> : active === "Feedback" ? <FeedbackPanel /> : active === "Live Operations" ? <LiveOperations /> : active === "Revenue Analytics" ? <RevenueAnalytics /> : active === "Settlements" ? <Settlements /> : active === "Risk & Fraud" ? <RiskFraud /> : active === "Roles & Permissions" ? <RolesPermissions /> : <AdminModuleView module={active} />}
          </div>
        </main>
      </div>
    </div>
