    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-20 shrink-0 items-center gap-3 border-b border-border bg-card/90 px-4 backdrop-blur-xl xl:px-7">
          ...
          <div className="hidden max-w-xl flex-1 items-center gap-2 rounded-xl border border-border bg-muted/50 px-3 md:flex"><Search className="h-4 w-4 text-muted-foreground" /><input className="h-10 w-full bg-transparent text-xs outline-none" placeholder="Search users, transactions, applications or tickets…" /><span className="rounded border border-border bg-card px-1.5 py-0.5 text-[9px] text-muted-foreground">⌘ K</span></div>
          ...
          <Button variant="outline" size="icon" className="relative"><Bell /><span className="absolute -right-1 -top-1 grid h-4 min-w-4 place-items-center rounded-full bg-admin-danger px-1 text-[8px] font-bold text-admin-danger-foreground">8</span></Button>
          <DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" className="h-10 gap-2 px-2"><span className="grid h-7 w-7 place-items-center rounded-lg bg-admin text-[10px] font-extrabold text-admin-foreground">SA</span><span className="hidden text-left sm:block"><span className="block text-[11px] font-bold">Super Admin</span><span className="block text-[8px] text-muted-foreground">Platform owner</span></span><ChevronDown /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem><Building2 />Organization</DropdownMenuItem><DropdownMenuItem><Settings />Preferences</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={signOut}><LogOut />Sign out</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
        </header>
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-[1700px] space-y-5 p-4 lg:p-6 xl:p-7">
            <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => setLastUpdated("Just now")}><RefreshCw /> Refresh <span className="hidden text-muted-foreground sm:inline">· {lastUpdated}</span></Button><Button size="sm" className="bg-admin text-admin-foreground hover:bg-admin/90"><BarChart3 /> Export report</Button></div>
            </section>
