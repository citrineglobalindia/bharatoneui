...
import {
  ...
  Network,
  type LucideIcon,
  ImageIcon, Megaphone } from "lucide-react";
import {
  Area,
  AreaChart,
  Bar,
...
const NAVIGATION: NavGroup[] = [
  ...
  },
  {
    label: "Finance",
    items: [
      { label: "Accountant Operations", icon: Landmark },
  ...
  },
  {
    label: "Platform",
    items: [
      { label: "Service Catalog", icon: Zap },
...
const VOLUME = [
  { time: "08:00", volume: 18, success: 16 },
  { time: "10:00", volume: 31, success: 29 },
  { time: "12:00", volume: 46, success: 42 },
  { time: "14:00", volume: 39, success: 37 },
  { time: "16:00", volume: 58, success: 54 },
  { time: "18:00", volume: 71, success: 68 },
  { time: "20:00", volume: 62, success: 59 },
];

const REGIONS = [
  { name: "Karnataka", value: 86 },
  { name: "Tamil Nadu", value: 72 },
  { name: "Maharashtra", value: 64 },
  { name: "Telangana", value: 58 },
...
const LIVE_ACTIVITY = [
  ...
  {
    id: "BO-92838",
    service: "AEPS Balance",
    user: "Namma One Center",
    location: "Mandya",
    amount: "—",
    status: "Review",
    time: "1 min ago",
...
const ALERTS = [
  {
    title: "Unusual wallet velocity",
    detail: "Retailer RT-2041 · 14 transfers in 6 minutes",
    level: "Critical",
    time: "2m",
  },
  {
    title: "KYC SLA threshold",
    detail: "34 applications waiting longer than 4 hours",
    level: "High",
    time: "8m",
  },
  {
    title: "Settlement variance",
    detail: "₹12,480 variance detected in Karnataka batch",
    level: "Medium",
    time: "22m",
  },
];
  ...
  return (
    <div className="flex h-full flex-col bg-admin-panel text-admin-panel-foreground">
      <div className="flex h-20 items-center gap-3 border-b border-admin-panel-foreground/10 px-5">
        <span className="rounded-xl bg-card p-1.5 shadow-soft">
          <BharatOneLogo size="sm" />
        </span>
        <div>
          <p className="font-display text-sm font-extrabold">BharatOne Control</p>
          <p className="text-[10px] text-admin-panel-foreground/55">Enterprise command center</p>
        </div>
      </div>
      <div className="mx-3 mt-4 rounded-xl border border-admin-panel-foreground/10 bg-admin-panel-foreground/5 p-3">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-bold uppercase tracking-widest text-admin-panel-foreground/55">
            Platform health
          </span>
          <span className="flex items-center gap-1 text-[10px] font-bold text-admin-success">
            <span className="h-1.5 w-1.5 rounded-full bg-admin-success animate-pulse" /> Operational
          </span>
        </div>
        <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-admin-panel-foreground/10">
          <div className="h-full w-[99%] rounded-full bg-admin-success" />
        </div>
        <p className="mt-2 text-[10px] text-admin-panel-foreground/45">
          99.98% uptime · 42ms response
        </p>
      </div>
      <nav className="flex-1 overflow-y-auto nav-scroll px-3 py-4">
        {NAVIGATION.map((group) => (
          <div key={group.label} className="mb-5">
            <p className="mb-1.5 px-3 text-[9px] font-extrabold uppercase tracking-[0.18em] text-admin-panel-foreground/35">
              {group.label}
            </p>
                    ...
                    {item.badge && (
                      <span
                        className={cn(
                          ...
                          selected
                            ? "bg-admin-foreground/15"
                            : "bg-admin-panel-foreground/10 text-admin-panel-foreground/60",
                        )}
                      >
                        {item.badge}
                      </span>
      ...
      <div className="border-t border-admin-panel-foreground/10 p-3">
        <div className="flex items-center gap-3 rounded-xl bg-admin-panel-foreground/5 p-2.5">
          <span className="grid h-9 w-9 place-items-center rounded-lg bg-admin font-display text-xs font-extrabold text-admin-foreground">
            {me.initials}
          </span>
          <div className="min-w-0 flex-1">
            <p className="truncate text-xs font-bold">{me.name}</p>
            <p className="truncate text-[9px] text-admin-panel-foreground/45">
              {me.role || "Full system access"}
            </p>
          </div>
          <ShieldCheck className="h-4 w-4 text-admin-success" />
        </div>
      </div>
    </div>
  );
...
function MetricCard({ item }: { item: (typeof KPI)[number] }) {
  ...
  return (
    <div className="group rounded-2xl border border-border bg-card p-4 shadow-soft transition hover:-translate-y-0.5 hover:shadow-elev">
      <div className="flex items-start justify-between">
        <span className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
          {item.label}
        </span>
        <span className={cn("grid h-9 w-9 place-items-center rounded-xl", tones[item.tone])}>
          <Icon className="h-4 w-4" />
        </span>
      </div>
      <p className="mt-3 font-display text-2xl font-extrabold tracking-tight">{item.value}</p>
      <p className="mt-1 text-[10px] font-semibold text-muted-foreground">{item.detail}</p>
    </div>
  );
}
...
export function AdminWorkspace() {
  ...
  const kpis = useMemo(() => {
    if (!dash) return KPI;
    const total = Number(dash.applications || 0),
      ok = Number(dash.approved || 0);
    return [
      {
        label: "Gross volume",
        value: inrShort(dash.gross_total),
        detail: inrShort(dash.gross_today) + " today",
        icon: BadgeIndianRupee,
        tone: "admin",
      },
      {
        label: "Retailers",
        value: Number(dash.retailers || 0).toLocaleString("en-IN"),
        detail: (dash.active_retailers || 0) + " active",
        icon: Store,
        tone: "success",
      },
      {
        label: "Wallet pool",
        value: inrShort(dash.wallet_pool),
        detail: "across wallets",
        icon: WalletCards,
        tone: "warning",
      },
      {
      ...
      {
        label: "KYC pending",
        value: Number(dash.kyc_pending || 0).toLocaleString("en-IN"),
      ...
      {
        label: "Open tickets",
        value: Number(dash.open_tickets || 0).toLocaleString("en-IN"),
  ...
  const servicesData = useMemo(() => {
    ...
    const arr = (dash?.service_dist as any[]) ?? [];
    if (!arr.length) return SERVICES;
    return arr.map((x, i) => ({
      name: x.name,
  ...
  const recentRows = useMemo(
    () =>
      ((dash?.recent as any[]) ?? []).map((r) => ({
        id: r.application_no,
        service: r.service_name,
        user: r.submitter_name || "\u2014",
        location: r.status,
        amount: "\u20b9" + Number(r.service_charge || 0).toLocaleString("en-IN"),
        status:
  ...
  const signOut = () => {
    ...
    localStorage.removeItem("bharatone:auth");
  ...
  const exportOverview = () => {
    downloadCsv(
      ...
      ["Metric", "Value", "Detail"],
      KPI.map((item) => [item.label, item.value, item.detail]),
    );
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 flex lg:hidden">
          <div
            className="absolute inset-0 bg-admin-panel/70 backdrop-blur-sm animate-in fade-in duration-200"
          ...
          <aside className="relative w-72 animate-in slide-in-from-left duration-200">
            <Button
              variant="ghost"
              size="icon"
              className="absolute right-2 top-2 z-10 text-admin-panel-foreground"
              onClick={() => setSidebarOpen(false)}
            >
              <X />
            </Button>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="h-10 gap-2 px-2">
                <span className="grid h-7 w-7 place-items-center rounded-lg bg-admin text-[10px] font-extrabold text-admin-foreground">
                  {me.initials}
                </span>
                <span className="hidden text-left sm:block">
                  <span className="block text-[11px] font-bold">{me.name}</span>
                  <span className="block text-[8px] text-muted-foreground">
                    {me.role || "Platform owner"}
                  </span>
                </span>
                <ChevronDown />
              </Button>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => setActive("User Management")}>
                <Building2 />
                Organization
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setActive("System Settings")}>
                <Settings />
                Preferences
              </DropdownMenuItem>
              <DropdownMenuSeparator />
          ...
          <div className="mx-auto max-w-[1700px] space-y-5 p-4 lg:p-6 xl:p-7">
            <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h1 className="mt-1 font-display text-2xl font-extrabold tracking-tight lg:text-3xl">
                  {active}
                </h1>
                <p className="mt-1 text-xs text-muted-foreground">
                  {date} · Real-time control across the BharatOne network
                </p>
              </div>
              ...
              <>
                ...
                <section className="grid gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(300px,.65fr)]">
                  <div className="rounded-2xl border border-border bg-card p-4 shadow-soft lg:p-5">
                    <div className="flex items-start justify-between">
                      <div>
                        <h2 className="text-sm font-extrabold">Transaction intelligence</h2>
                        <p className="text-[10px] text-muted-foreground">
                          Hourly gross volume and successful settlements · in lakhs
                        </p>
                      </div>
                      <span className="rounded-lg bg-admin-success-soft px-2 py-1 text-[9px] font-extrabold text-admin-success">
                        +18.4% TODAY
                      </span>
                    </div>
                    <div className="mt-4 h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={VOLUME} margin={{ left: -20, right: 5 }}>
                          <defs>
                            <linearGradient id="adminVolume" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <div className="rounded-2xl border border-border bg-card p-4 shadow-soft lg:p-5">
                    <div>
                      <h2 className="text-sm font-extrabold">Service distribution</h2>
                      <p className="text-[10px] text-muted-foreground">
                        Share of today's transaction volume
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {servicesData.map((item) => (
                        <div
                          key={item.name}
                          className="flex items-center justify-between rounded-lg bg-muted/50 px-2.5 py-2 text-[10px]"
                        >
                ...
                <section className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,.75fr)]">
                  <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
                    <div className="flex items-center justify-between border-b border-border px-4 py-3.5">
                      <div>
                        <h2 className="text-sm font-extrabold">Live network activity</h2>
                        <p className="text-[10px] text-muted-foreground">
                          Streaming platform transactions
                        </p>
                      </div>
                      <Button
                        variant="ghost"
                        ...
                        onClick={() => setActive("Live Operations")}
                      >
                        Open monitor <ArrowUpRight />
                      </Button>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full min-w-[720px] text-left">
                        <thead className="bg-muted/40 text-[9px] font-extrabold uppercase tracking-wider text-muted-foreground">
                          <tr>
                            {["Transaction", "Service", "Retailer", "Amount", "Status", "Time"].map(
                              (head) => (
                                <th key={head} className="px-4 py-2.5">
                                  {head}
                                </th>
                              ),
                            )}
                        ...
                        <tbody className="divide-y divide-border">
                          {(recentRows.length ? recentRows : LIVE_ACTIVITY).map((row) => (
                            <tr key={row.id} className="text-[11px] transition hover:bg-muted/30">
                              <td className="px-4 py-3 font-mono font-semibold">{row.id}</td>
                              <td className="px-4 py-3 font-bold">{row.service}</td>
                              <td className="px-4 py-3">
                                <p className="font-semibold">{row.user}</p>
                                <p className="text-[9px] text-muted-foreground">{row.location}</p>
                              </td>
                              <td className="px-4 py-3 font-extrabold">{row.amount}</td>
                              <td className="px-4 py-3">
                                <span
                                  className={cn(
                  ...
                  <div className="rounded-2xl border border-border bg-card p-4 shadow-soft">
                    <div className="flex items-center justify-between">
                      <div>
                        <h2 className="text-sm font-extrabold">Priority alerts</h2>
                        <p className="text-[10px] text-muted-foreground">
                          AI-assisted risk and operations queue
                        </p>
                      </div>
                      <span className="rounded-full bg-admin-danger-soft px-2 py-1 text-[9px] font-extrabold text-admin-danger">
                        3 OPEN
                      </span>
                    </div>
                    <div className="mt-3 space-y-2.5">
                      {ALERTS.map((alert) => (
                        <Button
                          key={alert.title}
                          variant="outline"
                          className="flex h-auto w-full items-start justify-start gap-3 whitespace-normal rounded-xl p-3 text-left"
                          onClick={() => {
                            setActive(
                              alert.title.includes("KYC")
                                ? "KYC Approvals"
                                : alert.title.includes("Settlement")
                                  ? "Settlements"
                                  : "Risk & Fraud",
                            );
                            toast.info(`Opened ${alert.title}`);
                          }}
                        >
                          <span
                            className={cn(
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="flex items-center justify-between">
                              <span className="text-[11px] font-extrabold">{alert.title}</span>
                ...
                <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                  <div className="rounded-2xl border border-border bg-card p-4 shadow-soft">
                    <div className="flex items-center justify-between">
                      <div>
                        <h2 className="text-sm font-extrabold">Regional performance</h2>
                        <p className="text-[10px] text-muted-foreground">
                          Activation target achievement by state
                        </p>
                      </div>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => setActive("Retailer Network")}
                      >
                    ...
                    <div className="mt-3 h-52">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={REGIONS} layout="vertical" margin={{ left: 4, right: 20 }}>
                  ...
                  <div className="rounded-2xl border border-border bg-admin-panel p-5 text-admin-panel-foreground shadow-elev">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-[9px] font-extrabold uppercase tracking-[0.18em] text-admin-panel-foreground/45">
                          Network pulse
                        </p>
                        <h2 className="mt-1 font-display text-xl font-extrabold">
                          Business is operating at peak efficiency.
                        </h2>
                      </div>
                      <span className="grid h-11 w-11 place-items-center rounded-xl bg-admin-success/15 text-admin-success">
                        <Activity />
                      </span>
                    </div>
                    <div className="mt-6 grid grid-cols-3 gap-3">
                      <div>
                        <p className="font-display text-2xl font-extrabold">99.98%</p>
                        <p className="text-[9px] text-admin-panel-foreground/45">Uptime</p>
                      </div>
                      <div>
                        <p className="font-display text-2xl font-extrabold">42ms</p>
                        <p className="text-[9px] text-admin-panel-foreground/45">API latency</p>
                      </div>
                      <div>
                        <p className="font-display text-2xl font-extrabold">0.03%</p>
                        <p className="text-[9px] text-admin-panel-foreground/45">Error rate</p>
                      </div>
                    </div>
                    <div className="mt-5 flex items-center gap-2 rounded-xl bg-admin-panel-foreground/5 p-3">
                      <ShieldCheck className="h-4 w-4 text-admin-success" />
                      <p className="text-[10px] text-admin-panel-foreground/65">
                        Security controls active · Last integrity scan passed 4 minutes ago
                      </p>
                    </div>
                  </div>
                </section>
