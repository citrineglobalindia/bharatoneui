...
import { Activity, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Clock3, Download, Filter, Search, TriangleAlert } from "lucide-react";
...
export function AttendanceLedger() {
  ...
  return <div className="space-y-5">
    <header className="relative overflow-hidden rounded-2xl bg-navy p-6 text-hr-foreground shadow-elev"><div className="absolute -right-14 -top-20 h-52 w-52 rounded-full bg-hr/25 blur-3xl"/><div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><div className="mb-3 grid h-11 w-11 place-items-center rounded-xl border border-hr-foreground/15 bg-hr-foreground/10"><CalendarDays className="h-5 w-5" /></div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-hr-foreground/55">Workforce time intelligence</p><h1 className="font-display text-2xl font-extrabold lg:text-3xl">Attendance Command Ledger</h1><p className="mt-1 text-sm text-hr-foreground/65">Daily status, payroll totals, exceptions and monthly workforce compliance.</p></div><div className="flex items-center gap-1 rounded-xl border border-hr-foreground/15 bg-hr-foreground/10 p-1"><Button variant="ghost" size="icon" className="text-hr-foreground hover:bg-hr-foreground/10 hover:text-hr-foreground" aria-label="Previous month" onClick={() => setMonthIndex((value) => Math.max(0, value - 1))}><ChevronLeft /></Button><span className="min-w-36 text-center text-sm font-bold">{monthLabel}</span><Button variant="ghost" size="icon" className="text-hr-foreground hover:bg-hr-foreground/10 hover:text-hr-foreground" aria-label="Next month" onClick={() => setMonthIndex((value) => Math.min(11, value + 1))}><ChevronRight /></Button></div></div></header>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{[{ label: "Present entries", value: summary.present, tone: "text-india-green", icon: CheckCircle2, note:"92.4% compliance" }, { label: "Absent entries", value: summary.absent, tone: "text-destructive", icon: TriangleAlert, note:"3 need review" }, { label: "Leave entries", value: summary.leave, tone: "text-hr", icon: CalendarDays, note:"All approved" }, { label: "Half days", value: summary.halfDay, tone: "text-saffron", icon: Clock3, note:"2 unregularized" }].map((item) => <div className="rounded-2xl border border-border bg-card p-5 shadow-soft" key={item.label}><div className="flex items-start justify-between"><div><p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{item.label}</p><p className={`mt-1 font-display text-2xl font-extrabold ${item.tone}`}>{item.value}</p></div><span className="grid h-9 w-9 place-items-center rounded-xl bg-hr-soft text-hr"><item.icon className="h-4 w-4"/></span></div><p className="mt-2 text-[10px] text-muted-foreground">{item.note}</p></div>)}</div>
    <div className="grid gap-4 xl:grid-cols-[1fr_300px]"><section className="overflow-hidden rounded-2xl border border-border bg-card shadow-soft"><div className="space-y-3 border-b border-border p-5"><div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-hr">Payroll-ready register</p><h2 className="font-display text-base font-extrabold">Monthly attendance ledger</h2><p className="text-[11px] text-muted-foreground">{rows.length} employees · Jun 1–30, 2026 · last synced 09:42 IST</p></div><div className="flex flex-col gap-2 sm:flex-row"><div className="flex h-9 items-center gap-2 rounded-xl border border-input px-3"><Search className="h-4 w-4 text-muted-foreground" /><input className="w-full bg-transparent text-sm outline-none sm:w-44" placeholder="Search employee…" value={query} onChange={(event) => setQuery(event.target.value)} /></div><Select value={department} onValueChange={setDepartment}><SelectTrigger className="w-full rounded-xl sm:w-44"><Filter className="h-3.5 w-3.5" /><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All departments</SelectItem>{Array.from(new Set(employees.map((employee) => employee.department))).map((item) => <SelectItem value={item} key={item}>{item}</SelectItem>)}</SelectContent></Select><Button variant="outline" className="rounded-xl" onClick={exportCsv}><Download /> Export CSV</Button></div></div>
    ...
    </section><aside className="space-y-4"><section className="rounded-2xl border border-border bg-card p-5 shadow-soft"><Activity className="text-hr"/><h2 className="mt-3 font-display text-base font-extrabold">Exception queue</h2><p className="text-xs text-muted-foreground">Items requiring HR regularization before payroll lock.</p><div className="mt-4 space-y-3">{[["Missing punch","3 employees","High"],["Late arrival","6 employees","Review"],["Shift mismatch","2 employees","Review"]].map(([title,count,status])=><button className="flex w-full items-center gap-3 rounded-xl bg-muted/50 p-3 text-left hover:bg-hr-soft" key={title}><span className={`h-2.5 w-2.5 rounded-full ${status==="High"?"bg-destructive":"bg-saffron"}`}/><span className="flex-1"><b className="block text-xs">{title}</b><span className="text-[10px] text-muted-foreground">{count}</span></span><ChevronRight className="h-4 w-4"/></button>)}</div><Button className="mt-4 w-full bg-hr text-hr-foreground hover:bg-hr/90" onClick={()=>toast.success("Regularization queue opened")}>Review exceptions</Button></section><section className="rounded-2xl bg-hr-soft p-5"><p className="text-[10px] font-bold uppercase tracking-wider text-hr">Payroll lock</p><p className="mt-1 font-display text-xl font-extrabold">28 June</p><p className="mt-1 text-xs text-muted-foreground">4 days remaining to finalize attendance.</p></section></aside></div>
  </div>;
}