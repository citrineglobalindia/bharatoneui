...
export function HrModulePage({ module }: { module: HrModuleKey }) {
  ...
  const runAction=()=>{ setActionOpen(false); toast.success(`${config.action} workflow started`,{description:"Workflow created successfully."}); };
  return <HrShell><div className="mx-auto max-w-[1600px] space-y-6">
    ...
    {module === "employees" ? <EmployeeOnboarding open={actionOpen} onOpenChange={setActionOpen} /> : <Dialog open={actionOpen} onOpenChange={setActionOpen}><DialogContent><DialogHeader><DialogTitle>{config.action}</DialogTitle><DialogDescription>Start a new {config.title.toLowerCase()} workflow.</DialogDescription></DialogHeader><div className="grid gap-3"><label className="text-xs font-bold">Title / Employee / Reference<input className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 text-sm font-normal outline-none focus:ring-1 focus:ring-hr" placeholder="Enter details" maxLength={120}/></label><label className="text-xs font-bold">Notes<textarea className="mt-1 min-h-24 w-full rounded-md border border-input bg-background p-3 text-sm font-normal outline-none focus:ring-1 focus:ring-hr" placeholder="Add workflow notes" maxLength={500}/></label></div><DialogFooter><Button variant="outline" onClick={()=>setActionOpen(false)}>Cancel</Button><Button className="bg-hr text-hr-foreground hover:bg-hr/90" onClick={runAction}>Create workflow</Button></DialogFooter></DialogContent></Dialog>}
