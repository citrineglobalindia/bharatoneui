...
function HrDashboard() {
  ...
  return <HrShell><div id="overview" className="mx-auto max-w-[1600px] space-y-7">
    <section className="relative overflow-hidden rounded-3xl bg-navy p-6 text-hr-foreground shadow-elev lg:p-8">
      ...
      <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-center"><div><span className="inline-flex rounded-full border border-hr-foreground/10 bg-hr-foreground/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-hr-foreground/65">Sunday, 14 June 2026 · Live workforce pulse</span><h1 className="mt-4 font-display text-3xl font-extrabold sm:text-4xl">Good morning, Ananya</h1><p className="mt-2 max-w-lg text-sm text-hr-foreground/65">Monitor workforce health, resolve priority approvals, and keep every HR cycle on track.</p><div className="mt-5 flex flex-wrap gap-2"><Button size="sm" className="bg-hr text-hr-foreground hover:bg-hr/90" onClick={()=>navigate({to:"/hr/employees"})}>Open employee command center</Button><Button size="sm" variant="outline" className="border-hr-foreground/20 bg-hr-foreground/5 text-hr-foreground hover:bg-hr-foreground/10 hover:text-hr-foreground" onClick={()=>navigate({to:"/hr/reports"})}>View executive report</Button></div></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[{l:"Workforce",v:"250",i:UsersRound,n:"+3 this month"},{l:"Active",v:"242",i:UserCheck,n:"96.8%"},{l:"Joiners",v:"12",i:UserPlus,n:"7 onboarded"},{l:"Exits",v:"8",i:TrendingDown,n:"-2 vs May"}].map(x => <div key={x.l} className="min-w-32 rounded-2xl border border-hr-foreground/10 bg-hr-foreground/10 p-4 backdrop-blur"><div className="flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-wider text-hr-foreground/55">{x.l}</p><x.i className="h-4 w-4 text-hr"/></div><p className="mt-3 text-2xl font-extrabold">{x.v}</p><p className="mt-1 text-[10px] text-hr-foreground/50">{x.n}</p></div>)}</div></div>
    </section>

    <section aria-label="Quick actions"><div className="mb-3 flex items-end justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-hr">Command Center</p><h2 className="font-display text-lg font-extrabold">Priority actions</h2></div><p className="hidden text-xs text-muted-foreground sm:block">5 shortcuts · 19 items awaiting action</p></div><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">{[
    ...
    ].map(([label, Icon], index) => <Button key={label as string} variant="outline" className="h-20 justify-start rounded-2xl bg-card px-4 shadow-soft hover:-translate-y-0.5 hover:border-hr hover:bg-hr-soft" onClick={() => navigate({ to: ["/hr/employees","/hr/leave","/hr/payroll","/hr/reports","/hr/employees"][index] })}><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-hr-soft text-hr"><Icon className="h-5 w-5" /></span><span className="text-left"><span className="block text-xs font-bold">{label as string}</span><span className="mt-1 block text-[9px] font-normal text-muted-foreground">Open workflow</span></span></Button>)}</div></section>
