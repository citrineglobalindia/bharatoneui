...
function AepsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setStatus(st);
      if (st?.settlement_enabled && st?.service_activated) void loadCashout();
    } catch { /* ignore */ }
    try {
  ...
  return (
    <RetailerShell>
      <div className="space-y-5">
        ...
        {/* AEPS Cashout — direct Eko fund settlement to the agent's own bank ("My Merchant" mode) */}
        {status?.settlement_enabled && status?.service_activated && (
          <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            <div className="mb-3 flex items-center justify-between">
