-- Store the Old JSKO ID (the JSKO username used during migration) on the retailer
-- registration so the accountant review can show it as the Agent ID.
alter table public.retailer_registrations add column if not exists jsko_id text;
...
begin
  ...
  insert into public.retailer_registrations (
    application_id, transaction_id, jsko_id, registration_type, status,
    ...
    pan_doc_path, aadhaar_doc_path, shop_photo_path, shop_photo_inside_path, police_verification_path, passport_photo_path,
    ...
    v_app_id, v_txn_id, nullif(payload->>'jsko_id',''),
    ...
    nullif(payload->>'shop_photo_path',''), nullif(payload->>'shop_photo_inside_path',''), nullif(payload->>'police_verification_path',''), nullif(payload->>'passport_photo_path',''),
